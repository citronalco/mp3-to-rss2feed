...
def main():
    parser = argparse.ArgumentParser(
        ...
        description = "Create a RSS 2.0 compliant podcast feed from MP3 files in a directory",
        epilog = """Example:
    %(prog)s /data/public/yoga-sessions-2025/ 'My Yoga Podcast 2025' https://example.com/yoga-sessions-2025/
    ...
    """)
    parser.add_argument("-p", "--private", help='Marks the feed as private (default: %(default)s)', default=False, action='store_true')
    parser.add_argument("directory", help='Directory containing the mp3 files')
    parser.add_argument("title", help='Title of your feed/podcast')
    parser.add_argument("url", help='URL to the directory containing the mp3 files')
    parser.add_argument("imageUrl", help='URL to an image for your feed/podcast (Optional)', nargs='?')
    args = parser.parse_args()

    PRIVATE = args.private
    DIR = args.directory
    FEEDTITLE = args.title
    URLBASE = args.url
    IMAGE = args.imageUrl
    ...
    ### save feed
    with open(path.join(DIR, 'podcast.xml'), "w", encoding="utf-8") as file:
